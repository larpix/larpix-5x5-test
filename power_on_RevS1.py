...
def main(vdda, vddd, io_group=1, pacman_tile=1, mclk=4, uart_clock_ratio=5, verbose=True):
    ...
    if True:
        ...
        print('Setting mclk to: ', mclk)
        c.io.set_reg(0x101c, mclk, io_group)
        c.io.set_uart_clock_ratio(IO_CHAN, mclk+1)
        time.sleep(0.015)
        c.io.set_uart_clock_ratio(IO_CHAN+1, mclk+1)
        time.sleep(0.015)
        c.io.set_uart_clock_ratio(IO_CHAN+2, mclk+1)
        time.sleep(0.015)
        c.io.set_uart_clock_ratio(IO_CHAN+3, mclk+1)
        ...
        c.io.set_reg(0x24131+2*(PACMAN_TILE-1), VDDD_DAC, io_group)
        c.io.set_reg(0x24130+2*(PACMAN_TILE-1), VDDA_DAC, io_group)
...
if __name__ == '__main__':
        ...
        parser.add_argument('--mclk', default=4, type=int, help='''pacman mclk''')
        parser.add_argument('--uart_clock_ratio', default=5, type=int, help='''pacman mclk''')
